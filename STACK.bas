...
* Author : Ian McGowan
* Date   : 06/13/89
* Edited : $Id: STACK 17349 2014-09-08 19:58:43Z mcgowi96 $
* Comment: Stacks TCL commands
***************************************************************************
EQU NUL TO '',SPC TO ' ',TRUE TO 1, FALSE TO 0
EQU BANG TO '~'
EQU UNIX TO '!'
...
GET.INPUT:
    IF GET.LINE.FLAG THEN
        CALL GET.LINE.STACK(X,LEN,DISP.LEN,ANS,RTN)
    END ELSE
        PRINT @(X):;INPUT ANS
        RTN = RET
    END
    ...
    IF INDEX(R,STAMP.STRING,1) THEN
        ...
        FOR N=1 TO 10
            IF R<N>[1,L] = STAMP.STRING THEN
                R<N> = STAMP.STRING:' ':TIMEDATE():' By ':USERNAME'L#8'
                WRITE R ON F,B.ITEM
                R = NUL
            END
        NEXT F
